...
layout = {
    'output' : ['Materiale','Descrizione doc.','UM','Quantità','Numero',
                ...
                'Numero OdV','Pos. OdV','Dt. consegna OdV','colore','finitura','altezza','larghezza','Qtà B2B','testo']
}
...
zsd67['Data documento'] = [dt.date(zsd67['Data documento'].iloc[i]) for i in range(len(zsd67))]
zsd67['Data consegna'] = [dt.date(zsd67['Data consegna'].iloc[i]) for i in range(len(zsd67))]
zsd67['Dt. consegna OdV'] = [dt.date(zsd67['Dt. consegna OdV'].iloc[i]) for i in range(len(zsd67))]
