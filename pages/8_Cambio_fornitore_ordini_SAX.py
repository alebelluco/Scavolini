...
path_flat = st.file_uploader('Caricare codici speciali')
if not path_flat:
    st.stop()



flat = pd.read_excel(path_flat, sheet_name='SAX')
colori = pd.read_excel(path_flat, sheet_name='colori')
colori
...
fornitori_change = colori.copy()
fornitori_change['key'] = [stringa[:10] for stringa in fornitori_change.colore.astype(str)]
fornitori_change['key'] = fornitori_change['key'] + '-'+ fornitori_change['finitura']
fornitori_change = fornitori_change[['key','Fornitore']]
...
spec_all['key'] = [stringa[:10] for stringa in spec_all.colore.astype(str)]
spec_all['key'] = spec_all['key'] + '-'+ spec_all['finitura']
spec_all = spec_all.merge(fornitori_change, how='left', left_on='key', right_on='key')
